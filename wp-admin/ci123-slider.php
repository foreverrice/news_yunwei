...
$sliderDataEdit = dirname(dirname(__FILE__)).'/wp-content/themes/news.ci123.com/glbRightSliderDataEdit.cache';
$sliderDataCache = dirname(dirname(__FILE__)).'/wp-content/themes/news.ci123.com/glbRightSliderData.cache';
...
if (isset($_POST['save']))
{
	...
	fputs($fp, $output);
	fclose($fp);
	echo "<script>window.location='http://news.ci123.com/wp-admin/ci123-slider.php';</script>";
}
...
<style type="text/css">
	.imgUpload { width: 250px; height: 330px; border: 3px dashed #DCDCDC; font-size: 20px; font-weight: bold; font-family: "黑体"; color: #DCDCDC; text-align: center; line-height: 330px; }
...
<?php for ($i = 0; $i < $num; $i++) { ?>
	var tpl_box<?=$i?> = '<div class="progress<?=$i?>"><img id="coverImg<?=$i?>" src="" width="250" height="330" /><input type="hidden" id="coverImgSrc<?=$i?>" name="cover<?=$i?>" value="" /></div>';
<?php } ?>
...
<form name="myform" method="POST" action="">
	...
	<div class="box">
		...
		<p><span>滚动周期：</span><input name="time" value="<?=$time?>" class="ipt2" /><span style="margin-left: 10px;">毫秒</span></p>
		<p><span>图片尺寸：250*330 请自行PS处理压缩至合适尺寸与大小</span></p>
	</div>
	...
	<div class="box fl">
		...
		<div id="imgUpload<?=$i?>" class="imgUpload ">
			<?php if ($imgs[$i]) { ?>
				<img src="<?=$imgs[$i]?>" width="250" height="330" />
