...
$sliderDataEdit = dirname(dirname(__FILE__)).'/wp-content/themes/news.ci123.com/albumSliderDataEdit.cache';
$sliderDataCache = dirname(dirname(__FILE__)).'/wp-content/themes/news.ci123.com/albumSliderData.cache';
$sliderData = unserialize(file_get_contents($sliderDataEdit));

$num = $sliderData['num'] ? $sliderData['num'] : 5;
...
if (isset($_POST['save']))
{
	...
	fputs($fp, $output);
	fclose($fp);
	echo "<script>window.location='http://news.ci123.com/wp-admin/ci123-photo-slider.php';</script>";
}
...
<style type="text/css">
	.imgUpload { width: 690px; height: 460px; border: 3px dashed #DCDCDC; font-size: 20px; font-weight: bold; font-family: "黑体"; color: #DCDCDC; text-align: center; line-height: 460px; }
	...
	.ipt { border-color: #CCC; padding: 3px 8px; font-size: 14px; line-height: 100%; width: 678px; outline: 0; -webkit-border-radius: 3px; border-radius: 3px; border-width: 1px; border-style: solid; color: #333; margin: 10px 0 0 0; }
...
<?php for ($i = 0; $i < $num; $i++) { ?>
	var tpl_box<?=$i?> = '<div class="progress<?=$i?>"><img id="coverImg<?=$i?>" src="" width="690" height="460" /><input type="hidden" id="coverImgSrc<?=$i?>" name="cover<?=$i?>" value="" /></div>';
<?php } ?>
...
<form name="myform" method="POST" action="">
	...
	<div class="box">
		...
		<p><span>图片尺寸：690*460 (9:6) 请自行PS处理压缩至合适尺寸与大小</span></p>
	</div>
	...
	<div class="box fl">
		...
		<div id="imgUpload<?=$i?>" class="imgUpload ">
			<?php if ($imgs[$i]) { ?>
				<img src="<?=$imgs[$i]?>" width="690" height="460" />
