...
<?php
	...
	function display_rows() {
		...
		if ( ! $this->is_site_users )
			...
			//$draft_counts = count_many_users_posts( array_keys( $this->items ), 'draft' );
		...
		foreach ( $this->items as $userid => $user_object ) {
			...
			//echo "\n\t", $this->single_row( $user_object, $style, $role, isset( $post_counts ) ? $post_counts[ $userid ].'/'.($post_counts[ $userid ] + $draft_counts[ $userid ]) : 0 );
		}
	}
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			switch ( $column_name ) {
				...
				case 'posts':
					...
					$r .= "<td $attributes>";
					global $wpdb;
					$total_posts = $wpdb->get_results("SELECT COUNT(`ID`) AS count FROM {$wpdb->prefix}posts WHERE `post_author` = '".$user_object->ID."' AND `post_status` IN ('publish', 'draft', 'future')");
					//var_dump($total_posts[0]->count);
					//echo "SELECT COUNT(`ID`) FROM {$wpdb->prefix}posts WHERE `post_author` = '".$user_object->ID."' AND `post_status` IN ('publish', 'draft')";
					if ( $numposts > 0 ) {
						$r .= "<a href='edit.php?author=$user_object->ID' title='" . esc_attr__( 'View posts by this author' ) . "' class='edit'>";
						$r .= $numposts.'/'.$total_posts[0]->count;
						$r .= '</a>';
					} else {
						$r .= 0;
					}
