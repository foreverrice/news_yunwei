...
$sliderDataEdit = dirname(dirname(__FILE__)).'/wp-content/themes/news_v3/socialRightPhotos2DataEdit.cache';
$sliderDataCache = dirname(dirname(__FILE__)).'/wp-content/themes/news_v3/socialRightPhotos2Data.cache';
...
if (isset($_POST['save']))
{
	...
	fputs($fp, $output);
	fclose($fp);
	echo "<script>window.location='http://news.ci123.com/wp-admin/ci123-social-right-photo2.php';</script>";
}
...
<style type="text/css">
	.imgUpload { width: 111px; height: 86px; border: 3px dashed #DCDCDC; font-size: 14px; font-weight: bold; font-family: "黑体"; color: #DCDCDC; text-align: center; line-height: 86px; }
...
<?php for ($i = 0; $i < $num; $i++) { ?>
	var tpl_box<?=$i?> = '<div class="progress<?=$i?>"><img id="coverImg<?=$i?>" src="" width="111" height="86" /><input type="hidden" id="coverImgSrc<?=$i?>" name="cover<?=$i?>" value="" /></div>';
<?php } ?>
...
<form name="myform" method="POST" action="">
	...
	<div class="box fl">
		...
		<div id="imgUpload<?=$i?>" class="imgUpload ">
			<?php if ($imgs[$i]) { ?>
				<img src="<?=$imgs[$i]?>" width="111" height="86" />
