...
$sliderDataEdit = dirname(dirname(__FILE__)).'/wp-content/themes/news_v3/socialRMTPDataEdit.cache';
$sliderDataCache = dirname(dirname(__FILE__)).'/wp-content/themes/news_v3/socialRMTPData.cache';
...
if (isset($_POST['save']))
{
	...
	fputs($fp, $output);
	fclose($fp);
	echo "<script>window.location='http://news.ci123.com/wp-admin/ci123-social-rementupian.php';</script>";
}
...
<style type="text/css">
	.imgUpload { width: 143px; height: 100px; border: 3px dashed #DCDCDC; font-size: 18px; font-weight: bold; font-family: "黑体"; color: #DCDCDC; text-align: center; line-height: 100px; }
...
<?php for ($i = 0; $i < $num; $i++) { ?>
	var tpl_box<?=$i?> = '<div class="progress<?=$i?>"><img id="coverImg<?=$i?>" src="" width="143" height="100" /><input type="hidden" id="coverImgSrc<?=$i?>" name="cover<?=$i?>" value="" /></div>';
<?php } ?>
...
<form name="myform" method="POST" action="">
	...
	<div class="box fl">
		...
		<div id="imgUpload<?=$i?>" class="imgUpload ">
			<?php if ($imgs[$i]) { ?>
				<img src="<?=$imgs[$i]?>" width="143" height="100" />
