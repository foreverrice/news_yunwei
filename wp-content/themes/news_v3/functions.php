...
function add_menus()
{
	...
	add_submenu_page( 'ci123-home-toutiao.php', '首页轮播', '首页轮播', 'administrator', 'ci123-home-lunbo.php', '');
	...
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 右侧图片一', 'administrator', 'ci123-social-right-photo1.php', '');
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 右侧图片二', 'administrator', 'ci123-social-right-photo2.php', '');
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 热门图片新闻', 'administrator', 'ci123-social-rementupian.php', '');
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 热门文章图片', 'administrator', 'ci123-social-remenwenzhang.php', '');
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 热搜词', 'administrator', 'ci123-social-resouci.php', '');
	add_submenu_page( 'ci123-home-toutiao.php', '社会新闻', '社会 轮播', 'administrator', 'ci123-social-slider.php', '');
...
//	echo "<p class=\"tips mgt10\">(本资讯由<a href=\"http://www.ci123.com/\" target=\"_blank\">育儿网</a>原创，转载请注明来自育儿网，并链回本页)</p>";
	echo "<p class=\"tips mgt10\">(原创作品，版权所有。未经授权，不得转载！)</p>";
    }
}
//add_theme_support( 'post-thumbnails' ); 
 ...
 * 获取上一篇下一篇文章
 */
function getPreviousAndNextPost()
{
	...
	{
		array_push($categoryIDS, $c->term_id);
	}
...
function pfSubStr($str, $len, $style = '...')
{
        preg_match_all('/(?:[\x80-\xff].)?./', $str, $matchs);
        $i = 0;
        foreach ($matchs[0] as $ma)
        {
                if (preg_match('/[^\x00-\x80]/', $ma))
                {
                        $i = $i + 2;
                }
                else
        ...
        if ($len > 0 && $len <= $i)
        {
                while ($j < $len)
                {
                        if (preg_match('/[^\x00-\x80]/', $matchs[0][$k]))
                        {
                                $tempstr = $tempstr.$matchs[0][$k];
                                $j=$j+2;
                        }
                        else
                        {
